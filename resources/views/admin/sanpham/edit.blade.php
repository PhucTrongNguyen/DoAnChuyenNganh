                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('sanpham.update', $sanPham->MaSP) }}" method="POST"
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <textarea class="form-control" name="MoTaChiTiet" id="MoTaChiTiet" style="height: 100px"
                                    >{{$sanPham->MoTaChiTiet}}</textarea>
                                ...
                                @if ($sanPham->GioiTinh == "Nam")
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="GioiTinh" id="GioiTinh"
                                            value="Nam" checked>
                                        <label class="form-check-label" for="GioiTinh" checked>
                                            Nam
                                        </label>
                                    </div>
                                    <div class="form-check">
                                ...
                                <select class="form-select {{ $errors->has('ThuongHieu') ? 'is-invalid' : '' }}" name="ThuongHieu" id="ThuongHieu"
                                    ...
                                    @foreach ($th as $item)
                                        <option value="{{ $item->MaTH }}" 
                                            {{ (old('ThuongHieu') == $item->MaTH || (isset($sanPham) && $sanPham->ThuongHieu == $item->MaTH)) ? 'selected' : '' }}>
                                            {{ $item->TenTH }}
                                        </option>
                                    @endforeach
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <select class="form-select {{ $errors->has('LoaiMatKinh') ? 'is-invalid' : '' }}" name="LoaiMatKinh" id="LoaiMatKinh"
                                    aria-label="Default select example">
                                    <option value="">Chọn loại mắt kính</option>
                                    @foreach ($lmk as $item)
                                        <option value="{{ $item->MaLoai }}" 
                                            {{ (old('LoaiMatKinh') == $item->MaLoai || (isset($sanPham) && $sanPham->LoaiMatKinh == $item->MaLoai)) ? 'selected' : '' }}>
                                            {{ $item->TenLoai }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <select class="form-select {{ $errors->has('Do') ? 'is-invalid' : '' }}" name="Do" id="Do">
                                    <option value="">Chọn độ mắt kính</option>
                                    @foreach ($do as $item)
                                        <option value="{{ $item->MaDo }}" 
                                        {{ (old('MaDo') == $item->MaDo || (isset($sanPham) && $sanPham->Do == $item->MaDo)) ? 'selected' : '' }}>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <select class="form-select {{ $errors->has('MaCLG') ? 'is-invalid' : '' }}" name="MaCLG" id="selectGong"
                                    aria-label="Default select example">
                                    <option value="">Chọn chất liệu gọng</option>
                                    @foreach ($tsg as $item)
                                        <option value="{{ $item->MaCLG }}" 
                                            {{ (old('MaCLG') == $item->MaCLG || (isset($sanPham) && $sanPham->MaCLG == $item->MaCLG)) ? 'selected' : '' }}
                                            data-mau="{{$item->MauGong}}"
                                            data-dai="{{$item->ChieuDaiGongKinh}}"
                                            data-rong="{{$item->ChieuRongGongKinh}}">
                                            {{ $item->TenCLG }}
                                        </option>
                                    ...
                                    @if ($sanPham->MaCLG == $item->MaCLG)
                                        ...
                                        <div class="col-sm-10">
                                            <input type="color" id="inputMauGong" class="form-control form-control-color"
                                                id="exampleColorInput" value="{{$item->MauGong}}" title="Choose your color"
                                                disabled>
                                        </div>
                                        ...
                                        <div class="col-sm-10">
                                            <input type="text" id="inputChieuDai" class="form-control"
                                                value="{{$item->ChieuDaiGongKinh}}" disabled>
                                        </div>
                                        ...
                                        <div class="col-sm-10">
                                            <input type="text" id="inputChieuRong" class="form-control"
                                                value="{{$item->ChieuRongGongKinh}}" disabled>
                                        </div>
                                    @endif
                                @endforeach
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <select class="form-select {{ $errors->has('MaCLT') ? 'is-invalid' : '' }}" name="MaCLT" id="selectTrong"
                                    aria-label="Default select example">
                                    <option value="">Chọn chất liệu tròng</option>
                                    @foreach ($tst as $item)
                                        <option value="{{ $item->MaCLT }}" 
                                            {{ (old('MaCLT') == $item->MaCLT || (isset($sanPham) && $sanPham->MaCLT == $item->MaCLT)) ? 'selected' : '' }}
                                            data-mau="{{$item->MauTrong}}"
                                            data-cao="{{$item->DoCaoTrong}}"
                                            data-rong="{{$item->DoRongTrong}}">
                                            {{ $item->TenCLT }}
                                        </option>
                                    @endforeach
                                </select>
                                    ...
                                    @if ($sanPham->MaCLT == $item->MaCLT)
                                        ...
                                        <div class="col-sm-10">
                                            <input type="color" id="inputMauTrong" class="form-control form-control-color"
                                                id="exampleColorInput" value="{{$item->MauTrong}}" title="Choose your color"
                                                disabled>
                                        </div>
                                        ...
                                        <div class="col-sm-10">
                                            <input type="text" id="inputDoCao" class="form-control"
                                                value="{{$item->DoCaoTrong}}" disabled>
                                        </div>
                                        ...
                                        <div class="col-sm-10">
                                            <input type="text" id="inputDoRong" class="form-control"
                                                value="{{$item->DoRongTrong}}" disabled>
                                        </div>
                                    @endif
                                @endforeach
                                ...
                                <select class="form-select {{ $errors->has('MaNCC') ? 'is-invalid' : '' }}" name="MaNCC" id="MaNCC" aria-label="Default select example">
                                    ...
                                    @foreach ($ncc as $item)
                                        <option value="{{ $item->MaNCC }}" 
                                            {{ (old('MaNCC') == $item->MaNCC || (isset($sanPham) && $sanPham->MaNCC == $item->MaNCC)) ? 'selected' : '' }}
                                            >
                                            {{ $item->TenNCC }}
                                        </option>
                                    @endforeach
